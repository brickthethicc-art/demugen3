import { IntentType, GamePhase, TurnPhase, STARTING_LIFE, ACTIVE_UNIT_COUNT, MAX_HAND_SIZE, } from '@mugen/shared';
...
export function resolveIntent(state, playerId, intent) {
    ...
    if (intent.type === IntentType.DISCARD_CARD && state.phase === GamePhase.IN_PROGRESS) {
        return resolveDiscardCard(state, playerId, intent);
    }
    ...
    switch (intent.type) {
        ...
        case IntentType.LOCK_TEAM: {
            ...
            if (allLocked) {
                ...
                const preGameState = { ...state, players: updatedPlayers };
...
function resolveDiscardCard(state, playerId, intent) {
