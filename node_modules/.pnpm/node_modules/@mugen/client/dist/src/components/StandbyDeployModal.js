...
export function StandbyDeployModal() {
    ...
    const activeUnitCount = myPlayer?.units.filter(u => u.position !== null &&
        u.position.x >= 0 &&
        u.position.x < (gameState?.board.width || 0) &&
        u.position.y >= 0 &&
        u.position.y < (gameState?.board.height || 0)).length ?? 0;
    ...
    return (_jsx(_Fragment, { children: notifVisible && (_jsx("div", { className: `fixed top-1/3 left-1/2 -translate-x-1/2 z-[9999] pointer-events-none transition-all duration-400 ${notifExiting
                ? 'opacity-0 -translate-y-12 scale-95'
                : 'opacity-100 translate-y-0 scale-100'}`, children: _jsx("div", { className: "bg-mugen-accent/95 text-white px-6 py-3 rounded-xl shadow-2xl text-sm font-semibold backdrop-blur-sm border border-mugen-accent/30", children: "Advance a bench unit to the board" }) })) }));
}
//# sourceMappingURL=StandbyDeployModal.js.map