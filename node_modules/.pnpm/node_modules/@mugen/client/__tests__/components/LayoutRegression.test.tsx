...
vi.mock('../../src/components/GameBoard.js', () => ({
  GameBoard: () => <div data-testid="game-board" style={{ width: '736px', height: '736px' }} />,
}));
vi.mock('../../src/components/GameHUD.js', () => ({
  GameHUD: () => <div data-testid="game-hud" />,
}));
vi.mock('../../src/components/HoverPanel.js', () => ({
  HoverPanel: () => <div data-testid="hover-panel" />,
}));
  ...
  describe('Deck & Discard Pile Positioning', () => {
    it('should position deck/discard piles between hand and bench', () => {
      ...
      if (container) {
        ...
        const deckDiscardContainer = children.find(child => 
          child.classList.contains('flex') && 
          child.classList.contains('flex-col') &&
          child.classList.contains('gap-2')
        );
        const benchElement = children.find(child => 
          child.getAttribute('data-testid') === 'bench-units-container'
        );
