...
describe('CombatEngine', () => {
  ...
  describe('getAttackTargets', () => {
    it('returns enemy units within range', () => {
      const attacker = createUnitInstance({
        card: { ...createUnitInstance().card, id: 'a1', range: 2 },
        position: { x: 5, y: 5 },
        ownerId: 'p1',
      ...
      expect(targets[0]!.ownerId).toBe('p2');
    });
    ...
    it('excludes friendly units', () => {
      ...
      const friendly = createUnitInstance({
        card: { ...createUnitInstance().card, id: 'f1' },
        position: { x: 6, y: 5 },
        ownerId: 'p1',
        currentHp: 5,
      });
      const targets = getAttackTargets(attacker, [attacker, friendly], 'p1');
    ...
    it('excludes dead units', () => {
      ...
      const deadEnemy = createUnitInstance({
        card: { ...createUnitInstance().card, id: 'e1' },
        position: { x: 6, y: 5 },
        ownerId: 'p2',
        currentHp: 0,
      });
      const targets = getAttackTargets(attacker, [attacker, deadEnemy], 'p1');
      expect(targets).toHaveLength(0);
    });

    it('excludes enemies out of range', () => {
      const attacker = createUnitInstance({
        card: { ...createUnitInstance().card, id: 'a1', range: 1 },
        position: { x: 5, y: 5 },
        ownerId: 'p1',
        hasAttackedThisTurn: false,
      });
      const farEnemy = createUnitInstance({
        card: { ...createUnitInstance().card, id: 'e1' },
        position: { x: 8, y: 5 },
        ownerId: 'p2',
        currentHp: 5,
      });
      const targets = getAttackTargets(attacker, [attacker, farEnemy], 'p1');
      expect(targets).toHaveLength(0);
    });

    it('returns empty if already attacked this turn', () => {
      const attacker = createUnitInstance({
        card: { ...createUnitInstance().card, id: 'a1', range: 2 },
        position: { x: 5, y: 5 },
        ownerId: 'p1',
    ...
    it('returns empty if attacker has no position', () => {
      const attacker = createUnitInstance({
        card: { ...createUnitInstance().card, id: 'a1', range: 2 },
        position: null,
        ownerId: 'p1',
