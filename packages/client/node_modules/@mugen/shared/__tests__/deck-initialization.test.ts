...
describe('Game Engine Deck Initialization', () => {
  it('should initialize player decks when creating game', () => {
    // Create a test deck
    ...
    // This should pass - decks are initialized with all 16 cards (no cards drawn at start)
    expect(gameResult.ok).toBe(true);
    if (gameResult.ok) {
      const game = gameResult.value;
      expect(game.players[0].deck.cards).toHaveLength(16); // All 16 cards remain in deck
      expect(game.players[1].deck.cards).toHaveLength(16); // All 16 cards remain in deck
    }
  });

  it('should start with empty hands (cards drawn on second turn)', () => {
    // Create a test deck
    const testDeckResult = createDeck([
      { id: 'card1', name: 'Test Card 1', cardType: CardType.UNIT, hp: 1, maxHp: 1, atk: 1, movement: 1, range: 1, ability: { id: 'ability1', name: 'Test Ability', description: 'Test', cost: 1, abilityType: 'DAMAGE' as any }, cost: 1 },
      { id: 'card2', name: 'Test Card 2', cardType: CardType.UNIT, hp: 2, maxHp: 2, atk: 2, movement: 2, range: 2, ability: { id: 'ability2', name: 'Test Ability', description: 'Test', cost: 2, abilityType: 'DAMAGE' as any }, cost: 2 },
    ...
    // This should pass - players start with empty hands
    expect(gameResult.ok).toBe(true);
    if (gameResult.ok) {
      const game = gameResult.value;
      expect(game.players[0].hand.cards).toHaveLength(0); // Empty hand at start
      expect(game.players[1].hand.cards).toHaveLength(0); // Empty hand at start
    }
  });
