import type { GameState, Position, Result, UnitInstance, UnitCard, PlayerColor } from '../../types/index.js';
...
export function placeStartingUnits(gameState: GameState, playerId: string): Result<GameState> {
  ...
  const color: PlayerColor = getPlayerColor(playerIndex);
  const startingPositions = getSpawnPositions(playerIndex, gameState.board.width, gameState.board.height);
  const reservePositions = getReservePositions(playerIndex, gameState.board.width, gameState.board.height);
  ...
  for (let i = 0; i < player.team.activeUnits.length; i++) {
    ...
    const unitInstanceId = `${playerId}-${unitCard.id}`;
  ...
  // Create bench/reserve unit instances (positioned in reserve area, not on main board)
  const benchInstances: UnitInstance[] = (player.team.reserveUnits ?? []).map((card, i) => ({
    card,
    currentHp: card.hp,
    position: reservePositions[i] ?? null,
    ownerId: playerId,
    color,
    hasMovedThisTurn: false,
    hasUsedAbilityThisTurn: false,
