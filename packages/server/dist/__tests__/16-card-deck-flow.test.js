...
describe('16-Card Deck Flow Integration', () => {
    ...
    it('complete flow: 16-card deck selection -> game start -> 10-card main deck', () => {
        ...
        const { active, bench } = StartingPlacementEngine.assignActiveAndBenchUnits(hostDeck.slice(0, 6));
        // Update host player's team
        const updatedPlayers = gameState.players.map(p => p.id === 'host-1'
            ? { ...p, team: { activeUnits: active, reserveUnits: bench, locked: true } }
            : p);
        const updatedState = { ...gameState, players: updatedPlayers };
        // Do the same for player 2
        const { active: active2, bench: bench2 } = StartingPlacementEngine.assignActiveAndBenchUnits(playerDeck.slice(0, 6));
        const finalPlayers = updatedState.players.map(p => p.id === 'player-2'
...
//# sourceMappingURL=16-card-deck-flow.test.js.map